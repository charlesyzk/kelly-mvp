...
针对外部《高阶矩动态 Kelly 策略》设计报告的逐项取舍见：[改进报告评审](docs/改进报告评审.md)。经验精确 Kelly 只用于诊断 M4 的 Taylor 近似误差，不是第二套交易策略。
...
- `signals.csv`：全部仓位信号，包含 RAW、M4有界解、经验精确诊断解以及最后一个 `pending` 当前信号；
- `periods.csv`：已经有下一期结果的信号、前四阶矩、仓位、收益、成本、对数增长和净值，可逐行复算；
...
诊断区额外显示：扣费后的平均单期与年化对数增长、M4 与经验精确 Kelly 的平均仓位差、方向一致率和 M4 触界率。最后一个没有未来收益的信号标记为 `pending`，只表示下一期目标仓位，不进入准确率或收益统计。
...
同一个窗口还会计算 `mean(log(1+fR_i))` 的经验精确最优解，用来回答“四阶近似与不截断经验目标相差多少”。它不会替代 M4 仓位。策略实际对数增长使用扣除已配置交易成本后的财富倍数计算；若财富倍数归零或为负，单期对数增长为空、年化几何增长为 `-100%`，不以人为小数替代 `log(0)`。
